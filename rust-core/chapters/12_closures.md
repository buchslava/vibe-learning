...
|---------------------|----------|
| Syntax, capture modes, `move`, `Fn` / `FnMut` / `FnOnce`, common traps | Async closures → Ch 16 |
| `.map`, `.sort_by`, `.retain`; `impl Fn` and `Box<dyn Fn()>` storage | `rayon` → Ch 14; HRTB, `Pin` → Ch 16 / 18 |
...
fn main() {
    let factor = 3;
    let scale = |x| x * factor;
    apply_bad(scale, 10); // ERROR: expected fn pointer, found closure
}
```
...
fn main() {
    let factor = 3;
    let scale = |x| x * factor;
    println!("{}", apply(scale, 10)); // ok — generic accepts any Fn
}
```
...
fn main() {
    ...
    for label in labels {
        fns.push(Box::new(move || println!("{}", label))); // ok — label is fresh each iteration
    }
    for f in &fns {
        f();
    }
    // labels vec is consumed by the loop; fns own the strings now
}
```
...
fn main() {
    ...
    let mut fns: Vec<Box<dyn Fn()>> = Vec::new();
    for label in &labels {
        fns.push(Box::new(|| println!("{}", label)));
        // ERROR: closure may run after loop — `label` ref dies at end of iteration
    }
}
```

Stored closures (`Box<dyn Fn()>`) can outlive the loop. A non-`move` closure **borrows** `label`; that reference ends when the iteration ends, but the closure still needs it later.
...
fn main() {
    ...
    for label in &labels {
        ...
        fns.push(Box::new(move || println!("{}", owned)));
    }
    for f in &fns {
        f();
    }
    println!("labels still here: {:?}", labels); // vec untouched
}
```
...
fn main() {
    ...
    for label in labels {
        ...
        println!("{}", label); // ERROR: label moved into closure above
    }
}
```
